...
# ------------------ helpers de columnas y reglas NASA ------------------
...
def _is_problematic(row: pd.Series) -> bool:
    # Reglas alineadas con los “Non-Metabolic Waste” problemáticos
    cat = str(row.get("category", "")).lower()
    fam = (str(row.get("material", "")) + " " + str(row.get("material_family", ""))).lower()
...
def _ensure_compat(df: pd.DataFrame) -> pd.DataFrame:
    ...
    id, category, material, kg, volume_l, flags, _problematic + trazabilidad.
    """
    out = df.copy()
    ...
    if kg_col != "kg":
        ...
        out["kg"] = pd.to_numeric(out["kg"], errors="coerce").fillna(0.0)
    ...
    if flg_col != "flags":
        ...
        out["flags"] = out["flags"].astype(str)
...
def _pick_materials(df: pd.DataFrame, n: int = 2) -> pd.DataFrame:
    ...
    # `sample` con weights requiere todos > 0
    w = w.fillna(0.01).clip(lower=0.01)
    n = min(n, len(df)) if len(df) > 0 else 0
    return df.sample(n=n, weights=w, replace=False, random_state=None) if n > 0 else df.head(0)

def _ensure_process_catalog(proc_df: pd.DataFrame) -> pd.DataFrame:
    """
    Normaliza columnas mínimas del catálogo de procesos y asegura que haya
    al menos una fila usable. Espera:
      process_id, name, energy_kwh_per_kg, water_l_per_kg, crew_min_per_batch
    """
    if proc_df is None or len(proc_df) == 0:
        return pd.DataFrame(columns=["process_id","name","energy_kwh_per_kg","water_l_per_kg","crew_min_per_batch"])

    out = proc_df.copy()
    # normalizar nombres
    rename = {}
    if "process_id" not in out.columns: rename[_col(out, ["id","process","proc_id"], None)] = "process_id"
    if "name" not in out.columns:       rename[_col(out, ["process_name","proc_name"], None)] = "name"
    if "energy_kwh_per_kg" not in out.columns: rename[_col(out, ["energy","energy_kwh"], None)] = "energy_kwh_per_kg"
    if "water_l_per_kg" not in out.columns:    rename[_col(out, ["water","water_l"], None)] = "water_l_per_kg"
    if "crew_min_per_batch" not in out.columns:rename[_col(out, ["crew_min","crew_time_min"], None)] = "crew_min_per_batch"
    # eliminar claves None del rename
    rename = {k:v for k,v in rename.items() if k is not None and k!=v}
    if rename:
        out = out.rename(columns=rename)

    # tipos numéricos seguros
    for c in ["energy_kwh_per_kg","water_l_per_kg","crew_min_per_batch"]:
        if c in out.columns:
            out[c] = pd.to_numeric(out[c], errors="coerce").fillna(0.0)
        else:
            out[c] = 0.0

    # process_id/name como string
    out["process_id"] = out["process_id"].astype(str)
    out["name"] = out["name"].astype(str)

    # si quedó vacío, devolver un dummy mínimo (evita bloqueo)
    if len(out) == 0:
        out = pd.DataFrame([{
            "process_id":"P00","name":"Fallback",
            "energy_kwh_per_kg":0.5,"water_l_per_kg":0.0,"crew_min_per_batch":15.0
        }])
...
def generate_candidates(
    ...
    - Insertar explícitamente **regolito MGS-1** cuando el proceso sea P03 (20% por defecto).
    """
    if waste_df is None or len(waste_df) == 0:
        return []
    proc_df = _ensure_process_catalog(proc_df)
    if len(proc_df) == 0:
        return []
    ...
    for _ in range(n):
        ...
        # Heurística: seleccionar proceso por características
        ...
        # Propiedades demo (intuitivas)
        rigidity  = min(1.0, 0.5 + (0.2 if (" al " in f" {mats_join} " or "aluminum" in mats_join) else 0.0))
        ...
        if str(proc["process_id"]).upper() == "P03":
            regolith_pct = 0.20  # 20% por defecto
            ...
            rigidity  = min(1.0, rigidity + 0.10)
        ...
        # Scoring multiobjetivo simple (transparente)
        def _pen(v, lim, eps):
            return max(0.0, (float(v) - float(lim)) / max(eps, float(lim)))
        ...
        # Bono por masa problemática valorizada
